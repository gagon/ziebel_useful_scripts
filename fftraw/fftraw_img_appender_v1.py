...
def make_imgs(plot_type,global_cnt,plot_vars,x_par,y_par,vmin,vmax,img_type):
    ...
    if '/%s' % plot_type not in img_hdf:
        img_hdf.create_group('/%s' % plot_type)       
    
    for i in plot_vars:
        if type(i) is list:
            if '/%s/%i_%i' % (plot_type,i[0],i[1]) in img_hdf:
                print "Dataset ",'/%s/%i_%i' % (plot_type,i[0],i[1])," exits!"
                continue
            else:
                img_hdf.create_group('/%s/%i_%i' % (plot_type,i[0],i[1])) 
        else:
            if '/%s/%i' % (plot_type,i) in img_hdf:
                print "Dataset ",'/%s/%i' % (plot_type,i)," exits!"
                continue
            else:
